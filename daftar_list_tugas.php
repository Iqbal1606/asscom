...
$sql = 'SELECT * FROM tugasmhs INNER JOIN mahasiswa on tugasmhs.usernamemhs = mahasiswa.usernamemhs
INNER JOIN tugas on tugasmhs.id_tugas = tugas.id_tugas';
    ...
    <nav class="navbar navbar-expand-lg navbar-light bg-warning fixed-top">
      ...
      <class class="row no-gutters mt-5 fixed-top">
          ...
          <class class="col-md-10 p-2 pt-4">
              <h3><i class="fas fa-users me-2"></i>List Tugas Mahasiswa</h3><hr class="bg-dark">
              <h5>DAFTAR ANGGOTA TERDAFTAR:</h5>
              <table class="table table-striped">
                <tr>
                  <th>NO</th>
                  <th>id tugas mahasiswa</th>
                  <th>nama tugas</th>
                ...
                while ($data = mysqli_fetch_array($query)) {
                      ?>
                          <tr>
                              <th><?php echo $no++ ?></th>
                              <th><?php echo $data['idtugas_mhs'] ?></th>
                              <th><?php echo $data['nama_tugas'] ?></th>
                              <th><?php echo $data['namamhs'] ?></th>
                              <th><?php echo $data['nim'] ?></th>
                              <th><a target= _blank href="/asscom/mahasiswa/file/<?php echo $data['filetgsmhs'] ?>"><?php echo $data['filetgsmhs'] ?></a></th>
                              <th><?php echo $data['tanggalpengumpulan'] ?></th>
                          </tr>
              <?php } ?>
